...
lv_obj_t * mybutton_create(lv_obj_t * parent, lv_color_t button_color, lv_opa_t button_opa, const void * icon, const char * text, lv_subject_t * subject, const char * fmt, int32_t ref_value)
{
    ...
    if (!style_inited) {
        ...
        lv_style_set_border_width(&style_normal, BORDER_WIDTH);
        lv_style_set_border_color(&style_normal, RED);
    ...
    lv_obj_set_name_static(lv_button_0, "mybutton_#");
    lv_obj_set_flex_flow(lv_button_0, LV_FLEX_FLOW_ROW);
    lv_obj_set_style_opa(lv_button_0, button_opa, 0);

    lv_obj_add_style(lv_button_0, &style_normal, 0);
    ...
    lv_label_set_translation_tag(lv_label_0, text);
    lv_obj_set_flag(lv_label_0, LV_OBJ_FLAG_SCROLLABLE, true);
    lv_obj_set_flag(lv_label_0, LV_OBJ_FLAG_HIDDEN, false);
    lv_obj_set_state(lv_label_0, LV_STATE_DISABLED, true);
    lv_obj_add_style(lv_label_0, &style_normal, LV_STATE_CHECKED);
